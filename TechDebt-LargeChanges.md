...
Companion to `TechDebt.md` items 2 and 3 (2026-07-17 scan). These two refactors are too large for an opportunistic `/debtpayer` pass — each touches multiple files or moves 150+ lines — so they are broken out here with a concrete plan and risk notes, pending a decision on how to handle them.
